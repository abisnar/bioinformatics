	...
	static int cmpfunc(const void* a,const void* b){
		...
		for (i = length-1; i > 0; i--){
			...
			if (permutations[i].string[0] == 'T') C_table[4] = i;
							...
							printf("\nCount_Table: Index of the first occurence of each character in the F column\n$-%d A-%d C-%d G-%d T-%d\n\n", C_table[0], C_table[1], C_table[2], C_table[3], C_table[4]);		
		...
		printf("\nBWT Sequence (The last column of the sorted permutations):\n\n%s\n\n", bwt_seq);  			//To print BWT sequence
							// printf("The suffix array: The original position of each permutation before sorting\n\n");
		...
		for (i = 0; i < length; i++){
			...
			BWT_Rank[4][i] = countT;			
							...
							printf("\n\nBWT      Rank Matrix:\n");
							for (i = 0; i < length; i++){
							printf("(%c) $-%d, A-%d, C-%d, G-%d, T-%d\n",bwt_seq[i], BWT_Rank[0][i], BWT_Rank[1][i], BWT_Rank[2][i], BWT_Rank[3][i], BWT_Rank[4][i]);
							}
